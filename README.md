...
1. 로컬 개발 서버를 실행합니다.
   ...
   기본 포트는 `5173`이며, 최초 실행 시 브라우저가 자동으로 열립니다. 자동 실행이 차단되면 수동으로 `http://localhost:5173/`에 접속하세요.
...
- 개발 서버는 `npm run dev`로 실행되며, Vite HMR 덕분에 파일 저장 시 즉시 반영됩니다.
- 프로덕션 번들은 `npm run build`로 생성되고 `dist/` 폴더에 저장됩니다. 완성된 번들은 `npm run preview`로 검수할 수 있습니다.
...
- Vite 기반 개발 서버와 번들 파이프라인을 도입해 `npm run dev`로 즉시 플레이하고, `npm run build`로 배포용 묶음을 만들 수 있게 했습니다.
- 메인 스크립트에서 스타일을 모듈로 불러오도록 조정해 번들 환경에서도 일관되게 CSS가 적용되도록 했습니다.
- README 실행 가이드를 로컬 개발 서버 중심으로 재작성하고, 새로운 워크플로에 맞춘 사용법을 추가했습니다.
